...
export async function GET() {
  ...
  const entries = signals
    .map((signal) => {
      const title = `${signal.direction} ${signal.pair} ${signal.timeframe} — rule score ${signal.confidence}/100`;
...
<h3>Recorded outcome labels (not broker fills)</h3>
<table cellpadding="4" cellspacing="0" border="1" style="border-collapse:collapse;font-family:monospace">
...
<hr/>
<p style="color:#666;font-size:12px">Generated by <a href="${BASE_URL}">TradeClaw</a> — MIT-licensed trading-signal software. Hosting and providers may charge separately.</p>
]]>`;
    ...
    <summary type="text">${escapeXml(`${signal.direction} ${signal.pair} at ${fmtPrice(signal.entryPrice)} | rule score ${signal.confidence}/100 (not a predictive probability)${signal.tp1 ? ` | TP1: ${fmtPrice(signal.tp1)}` : ''}${signal.sl ? ` | SL: ${fmtPrice(signal.sl)}` : ''}`)}</summary>
  </entry>`;
    })
    .join('\n');
...
<feed xmlns="http://www.w3.org/2005/Atom">
  ...
  <title type="text">TradeClaw Signal Archive</title>
  <subtitle type="text">Available TradeClaw signal records for configured markets. The rule score is mechanical indicator confluence, not a predictive probability; outcome labels are not broker fills.</subtitle>
