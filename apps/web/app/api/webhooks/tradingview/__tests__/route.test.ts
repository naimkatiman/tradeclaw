...
describe('POST /api/webhooks/tradingview', () => {
  ...
  it('persists a valid TradingView signal and returns ok', async () => {
    ...
    // Replayed webhooks stay idempotent via ON CONFLICT DO NOTHING.
    expect(mockedQueryOne.mock.calls[0][0]).toContain('ON CONFLICT (source_id) DO NOTHING');
  ...
  it('stays idempotent on a replayed (duplicate source_id) signal', async () => {
    // ON CONFLICT DO NOTHING absorbs the duplicate insert.
