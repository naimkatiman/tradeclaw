    ...
    >
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span
                className={`inline-block mt-0.5 text-[10px] px-2 py-0.5 rounded-full border ${CATEGORY_COLORS[integration.category]}`}
              >
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          {/* Recipe boundary */}
          <div className="p-3 rounded-lg bg-amber-500/5 border border-amber-500/20 text-xs text-amber-200/80 leading-relaxed">
            This catalog does not install or verify third-party connections. Configure the destination URL in Webhook
            Settings. Services that do not accept TradeClaw&apos;s standard JSON shape need an operator-owned relay or
            transform.
          </div>
          ...
          <div className="flex items-center justify-between gap-4 pt-2 border-t border-white/5">
            <div className="text-xs text-[var(--text-secondary)]">
              Use signal.test in Webhook Settings to verify a configured destination explicitly.
            </div>
...
function IntegrationCard({ integration, onInstall }: { integration: Integration; onInstall: () => void }) {
  return (
    <div className="glass-card rounded-2xl p-5 flex flex-col gap-4 group">
      ...
      {/* Recipe status */}
      <div className="p-2.5 rounded-lg bg-black/20 border border-white/5 text-[10px] text-zinc-500">
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium border border-emerald-500/20 bg-emerald-500/5 text-emerald-400">
            ...
            {INTEGRATIONS.length} setup recipes
          </div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">
            Webhook <span className="text-emerald-400">Recipe Catalog</span>
          </h1>
          <p className="text-[var(--text-secondary)] max-w-xl mx-auto text-sm leading-relaxed">
            Review manual setup paths for outbound webhook destinations. These are examples, not one-click native
            integrations, and some targets require an operator-owned relay.
          </p>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { value: `${INTEGRATIONS.length}`, label: 'Setup recipes' },
            { value: 'HTTPS', label: 'Destinations only' },
            { value: 'Manual', label: 'Verification' },
          ].map((s) => (
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((item) => (
              <IntegrationCard key={item.id} integration={item} onInstall={() => setActiveIntegration(item)} />
            ))}
          </div>
        ) : (
          <div className="text-center py-16 text-[var(--text-secondary)]">
            <div className="mb-3">
              <Search className="w-10 h-10 mx-auto text-[var(--text-secondary)]" />
            </div>
        ...
        <div className="glass-card rounded-2xl p-8 text-center space-y-4 border border-emerald-500/10">
          <div>
            <Wrench className="w-8 h-8 mx-auto text-emerald-400" />
          </div>
          <h2 className="text-lg font-semibold">Build Your Own Integration</h2>
          <p className="text-sm text-[var(--text-secondary)] max-w-md mx-auto">
            TradeClaw can send standard JSON to allowed HTTPS endpoints after an authenticated dispatch. Entry-like
            events also require the evidence gate; service-specific auth or schemas may need a relay.
          </p>
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-500/5 border border-blue-500/15">
          ...
          <div className="text-xs text-blue-300/80 leading-relaxed">
            <strong>For developers:</strong> Query the recipe catalog programmatically via{' '}
            <code className="text-blue-400">GET /api/marketplace</code>. Returns recipe metadata as JSON; it does not
            report installed or verified connections.
          </div>
        </div>
      </div>

      {/* Modal */}
      {activeIntegration && <InstallModal integration={activeIntegration} onClose={() => setActiveIntegration(null)} />}
    </main>
  );
}
