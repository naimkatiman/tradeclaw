...
interface LandingStats {
  /** Sum of pnlPct across real closed 24h outcomes in the last 30 days. */
  cumulativePnlPct: number;
  /** Sum(wins) / abs(sum(losses)); null when insufficient data to compute. */
  profitFactor: number | null;
  /** Count of signals generated since midnight UTC today. */
  signalsToday: number;
  /** Count of real closed 24h outcomes in the last 30 days. */
  closedSignals30d: number;
  /** Most recent signal (open or closed). */
  latestSignal: SignalPayload | null;
  /** Two variants of the most recent signal — free gets TP1 only and a +15min timestamp. */
...
export async function GET() {
  try {
    // --- 30d aggregate over CLOSED REAL outcomes on outcome_24h ---
    // "Real" = exclude auto-expire: NOT (pnlPct = 0 AND hit = false).
    // The engine writes {price, pnlPct:0, hit:false} when a signal expires
    // without hitting TP or SL. Those must not count toward P&L or profit factor.
