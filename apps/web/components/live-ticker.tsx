...
function TickerItem({ pair, tick }: TickerItemProps) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-[11px] font-mono shrink-0 select-none">
      <span className="text-zinc-500 dark:text-zinc-500">{SYMBOL_LABELS[pair] ?? pair}</span>
...
export function LiveTicker({ prices, pairs }: LiveTickerProps) {
  ...
  if (activePairs.length === 0) {
    return (
      <div className="bg-gray-50 dark:bg-[#050505] border-b border-gray-200 dark:border-white/5 h-10 flex items-center px-4">
        <div className="flex items-center gap-2">
          <span className="h-1.5 w-1.5 rounded-full bg-gray-300 dark:bg-zinc-700 animate-pulse" />
          <span className="text-[11px] text-gray-400 dark:text-zinc-700 font-mono">Connecting to market data…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-50 dark:bg-[#050505] border-b border-gray-200 dark:border-white/[0.06] h-10 overflow-hidden flex items-center">
      {/* LIVE badge */}
