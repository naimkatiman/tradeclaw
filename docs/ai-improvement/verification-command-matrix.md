...
| GREEN focused recost parser Jest | same focused Jest command | exit 0; 1 suite / 23 tests / 9.606s. | Covers help, missing/unknown/duplicate flags, unsafe numeric input, blank JSON paths, missing-parent JSON paths, existing-directory JSON paths, no-artifact behavior, and valid-input missing-DB boundary. |
