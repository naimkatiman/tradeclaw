...
export class TradeclawClient {
   ...
   */
  async signals(filter: SignalFilter = {}): Promise<Signal[]> {
    ...
    const data = await this.request<Signal[] | { signals: Signal[] }>(`/api/v1/signals${qs ? `?${qs}` : ''}`);
   ...
   */
  async leaderboard(filter: LeaderboardFilter = {}): Promise<LeaderboardEntry[]> {
    const params = new URLSearchParams();
    if (filter.period) params.set('period', filter.period);
    if (filter.sort) params.set('sort', filter.sort);
    const qs = params.toString();
    const data = await this.request<LeaderboardEntry[] | { leaderboard: LeaderboardEntry[] }>(
   ...
   */
  async badge(pair: string): Promise<{ schemaVersion: number; label: string; message: string; color: string }> {
