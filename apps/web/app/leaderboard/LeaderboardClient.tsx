...
function RankBadge({ rank }: { rank: number }) {
  ...
  if (rank === 2)
    return (
      <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-zinc-400/10 text-zinc-300 text-[10px] font-bold">
        2
      </span>
    );
  if (rank === 3)
...
function ConfBar({ value }: { value: number }) {
  return (
    <div className="flex items-center gap-2">
      <div className="relative flex-1 h-[3px] rounded-full bg-white/5">
        <div
...
function PairDetailPanel({ pair, onClose }: { pair: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="glass-card rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col border border-white/10">
        {/* header */}
