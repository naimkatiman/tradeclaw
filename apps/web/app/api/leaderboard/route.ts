...
export async function GET(request: NextRequest) {
  try {
    ...
    // Strategy-filtered and broadcast-scope variants recompute from history;
    // the unfiltered default path still uses the precomputed period:sort
    // cache. The broadcast scope MUST NOT fall through to the default cache —
    // it would silently serve firehose stats as the gated subset.
