...
export async function runExecutorTick(): Promise<ExecutorTickResult> {
  ...
  // Broker dispatch — keep Binance and R StocksTrader paths entirely separate.
  // Do NOT try to unify them behind an abstract interface on the first pass.
  const broker = (process.env.EXECUTION_BROKER ?? 'binance').toLowerCase();
  if (broker === 'r-stockstrader') {
    return runRStocksTraderTick(result);
  }
...
//
// Intentionally separate from the Binance path. Broker asymmetries (MT5 lot
// semantics, attached SL/TP, no separate cancel-entry path) make a shared
// abstraction more expensive than it saves.

const RST_BROKER = 'r-stockstrader';
const RST_TP1_FRACTION = 0.5;

async function runRStocksTraderTick(result: ExecutorTickResult): Promise<ExecutorTickResult> {
  const env = readRStocksTraderEnvOrNull();
  if (!env) {
    console.warn('[pilot/executor/rst] missing RSTOCKSTRADER_* env vars — skipping tick');
    result.errors++;
    return result;
  }
    ...
    try {
      ...
      if (inTickSymbols.has(symbol)) {
        ...
        await logRstError({
          ...
          stage: 'filter',
          errorCode: 'symbol_already_entered_in_tick',
          errorMsg: `${symbol} already entered in this tick`,
        });
        continue;
      }
      ...
      // Sizing: use Binance klines for ATR (market data only, no orders).
      // R StocksTrader doesn't have a klines endpoint we know of; Binance
      // mainnet candles are close enough for ATR calculation on these pairs.
      let spec;
      try {
      ...
      // Risk-first sizing: same 1% equity rule as Binance path.
      const equityUsd = accountInfo.equity;
      const riskPct = cfgInt('EXEC_RISK_PCT', 1) / 100;
      const riskUsd = equityUsd * riskPct;
      const stopDistance = atr * 1.5; // 1.5× ATR stop (matches Binance sizing.ts)
      ...
      const tp1Price = side === 'BUY'
        ? entryPrice + stopDistance * 3  // 3:1 R minimum (adjust if needed)
        : entryPrice - stopDistance * 3;

      // Validate stop distance against instrument minimum.
      if (stopDistance < spec.minStopDistance) {
        ...
        await logRstError({
          ...
          errorMsg: `stopDistance=${stopDistance} < minStopDistance=${spec.minStopDistance} for ${symbol}`,
        });
        continue;
      }

      // qty = riskUsd / (stopDistance * contractSize), rounded down to qtyStep
      const rawQty = riskUsd / (stopDistance * spec.contractSize);
      const qty = roundDownToQtyStep(rawQty, spec.qtyStep);

      // Per-asset notional cap
      const notionalPct = getNotionalCapPct(assetClass);
      const notionalUsd = qty * entryPrice * spec.contractSize;
      const notionalCapUsd = equityUsd * notionalPct;
      if (notionalUsd > notionalCapUsd) {
        result.rejected++;
        await logRstError({
          signalId: sig.id,
          stage: 'size',
          errorCode: 'notional_cap_exceeded',
          errorMsg: `notionalUsd=${notionalUsd.toFixed(2)} > cap=${notionalCapUsd.toFixed(2)} for ${symbol}`,
        });
        continue;
      }
      ...
      if (qty < spec.minQty) {
        ...
        await logRstError({
          ...
          errorMsg: `qty=${qty} < minQty=${spec.minQty} for ${symbol}`,
        });
        continue;
      }

      // Idempotency ref matches binance client-id pattern for cross-broker
      // consistency in the executions table.
      const ids = buildClientIds(sig.id);
      const placeInput: RStocksTraderPlaceInput = {
        ...
        type: 'MARKET',
        qty,
        stopLoss: roundToTick(stopPrice, spec.tickSize, spec.digits),
        takeProfit: roundToTick(tp1Price, spec.tickSize, spec.digits),
        clientRef: ids.entry,
      ...
      if (placed.status === 'rejected') {
        ...
        await logRstError({
          ...
          errorMsg: placed.rejectReason ?? 'broker rejected order',
        });
        continue;
      }
      ...
      await persistRstExecution({
        ...
        side,
        qty,
        entryPrice: placed.avgFillPrice ?? entryPrice,
        stopPrice: placeInput.stopLoss,
        tp1Price: placeInput.takeProfit,
        leverage: 1, // R StocksTrader reports leverage per instrument; 1 is a safe default
        notionalUsd,
        riskUsd,
        clientOrderId: ids.entry,
        exchangeOrderId: placed.brokerOrderId || null,
        status: placed.status === 'filled' ? 'filled' : 'pending',
        // SL/TP are attached to the position in a single order on this broker;
        // no separate order IDs to track.
        slOrderId: null,
        tpOrderId: null,
        mode: 'testnet', // demo account — always testnet until operator flips to live
      });
...
async function logRstError(a: ErrorLogArgs): Promise<void> {
  try {
    await execute(
      `INSERT INTO execution_errors
        (signal_id, execution_id, broker, stage, error_code, error_msg, payload)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [
        a.signalId ?? null, a.executionId ?? null, RST_BROKER,
        a.stage, a.errorCode ?? null, a.errorMsg.slice(0, 2000),
        a.payload ? JSON.stringify(a.payload) : null,
      ],
    );
...
// ─── R StocksTrader sizing helpers ───────────────────────────────────────────

function roundDownToQtyStep(qty: number, step: number): number {
...
/** Per-asset class notional cap as a fraction of equity. */
function getNotionalCapPct(assetClass: string): number {
  if (assetClass === 'crypto-cfd') {
    return cfgInt('EXEC_CRYPTO_CFD_PER_TRADE_NOTIONAL_PCT', 50) / 100;
  }
  if (assetClass === 'fx' || assetClass === 'metal' || assetClass === 'energy-cfd') {
    return cfgInt('EXEC_FX_PER_TRADE_NOTIONAL_PCT', 100) / 100;
  }
  return cfgInt('EXEC_STOCK_PER_TRADE_NOTIONAL_PCT', 100) / 100;
}
