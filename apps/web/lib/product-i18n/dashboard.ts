...
const en: DashboardTranslations = {
  common: {
    all: 'All', buy: 'BUY', sell: 'SELL', live: 'OBSERVED', demo: 'DEMO', copied: 'Copied!',
    ...
    bias: { bull: 'Up case', bear: 'Down case', neutral: 'Neutral' },
  },
  document: {
    title: 'TradeClaw — Research Candidate Feed',
    titleWithBuyCount: '({count} BUY labels) TradeClaw — Candidate Feed',
  },
  onboarding: {
  ...
  explanation: {
    title: 'Why this candidate appeared',
  ...
  levels: {
    entry: 'Observed price', sl: 'SL', tp1: 'TP1', tp2: 'TP2', tp3: 'TP3',
    entryHint: 'Provider price stamped when the candidate was generated.',
  ...
  history: {
    heading: 'Raw Candidate Outcome Record', winRate: 'win rate', resolved: 'resolved', streak: 'streak', priceMoveSum: 'price-move sum',
    ...
    priceMoveSumHint: 'Arithmetic sum of counted candidates’ OHLCV-resolved price moves, without position sizing or execution costs. It is not a portfolio return.',
    pair: 'Pair', direction: 'Dir', ruleScore: 'Rule score', entry: 'Entry',
    directionalMove: 'Directional move', viewRecord: 'Inspect the full track record →',
  },
  controls: { auto: 'Refresh 60s', paused: 'Refresh paused' },
  transparency: {
    providerBefore: 'Runtime quotes prefer',
    providerAfter: 'through the market-data hub. Candidate candles use the hub when configured, then Binance for crypto or Stooq for forex/metals; synthetic fallback rows are suppressed from this candidate list.',
    provenance: 'Card badges identify the expected provider lane; the real-versus-synthetic quality flag is the runtime provenance currently carried on each candidate.',
  },
  warning: {
    syntheticSymbols: '{count} of {total} symbols are using synthetic data (API unavailable) — candidates suppressed for those pairs.',
  },
  hero: { selected: 'SELECTED CANDIDATE', topSignal: 'TOP CANDIDATE', score: '{score}/100 rule score' },
  stats: { activeSignals: 'Current observations', buySell: 'Directional labels', averageRuleScore: 'Avg rule score', marketBias: 'Candidate bias' },
  filters: { highRuleScore: 'High Rule Score (≥80/100)', watchlist: 'Watchlist' },
  empty: {
    noAssetSignals: 'No {asset} candidates right now',
    noFilteredSignals: 'No candidates match the current filters',
    otherCategoriesForex: 'Candidates in other categories: {count}. Try another asset class or check back when forex sessions are open.',
    otherCategoriesMetals: 'Candidates in other categories: {count}. Try another asset class or check back when metals markets are active.',
    otherCategoriesOther: 'Candidates in other categories: {count}. Try another asset class or check back when more observations qualify.',
    adjustFilters: 'Current candidates: {count}. Adjust the score filters to inspect them.',
    showAllAssets: 'Show all assets',
    generating: 'Generating candidates…',
    analyzing: 'The rule engine is inspecting provider-backed market data. This takes up to 30 seconds on first load.',
    autoRetrying: 'Auto-retrying every 15s. Candidates appear when observations meet the rule threshold.',
    retryNow: 'Retry now',
  ...
  potential: {
    heading: 'Exploratory candidates', scoreBand: 'rule score 50–69/100', count: 'Observations: {count}',
    description: 'Early-stage observations with partial indicator agreement. They are included for comparison, not action.',
  },
  footer: {
    tagline: 'TradeClaw Research Lab · Open Source · Self-Hosted',
    disclaimer: 'Research candidates are for inspection only. Not trade instructions or financial advice.',
  },
};

const es: DashboardTranslations = {
  common: {
    all: 'Todos', buy: 'COMPRA', sell: 'VENTA', live: 'OBSERVADO', demo: 'DEMO', copied: '¡Copiado!',
    ...
    bias: { bull: 'Caso alcista', bear: 'Caso bajista', neutral: 'Neutral' },
  },
  document: { title: 'TradeClaw — Candidatos de investigación', titleWithBuyCount: '({count} etiquetas COMPRA) TradeClaw — Candidatos' },
  onboarding: {
  ...
  explanation: {
    title: '¿Por qué apareció esta candidata?', rsiOversold: 'RSI en {value} (condición de sobreventa)', rsiBelowNeutral: 'RSI en {value} (por debajo de neutral)',
  ...
  levels: {
    entry: 'Precio observado', sl: 'SL', tp1: 'TP1', tp2: 'TP2', tp3: 'TP3', entryHint: 'Precio del proveedor registrado cuando se generó la candidata.',
  ...
  history: {
    heading: 'Registro bruto de resultados candidatos', winRate: 'tasa de acierto', resolved: 'resueltas', streak: 'racha', priceMoveSum: 'suma de movimientos',
    ...
    priceMoveSumHint: 'Suma aritmética de los movimientos OHLCV de las candidatas contabilizadas, sin tamaño de posición ni costes de ejecución. No es una rentabilidad de cartera.',
    pair: 'Par', direction: 'Dir.', ruleScore: 'Puntuación', entry: 'Entrada',
    directionalMove: 'Movimiento direccional', viewRecord: 'Inspeccionar el historial completo →',
  },
  controls: { auto: 'Actualiza 60 s', paused: 'Actualización pausada' },
  transparency: {
    ...
    provenance: 'Las insignias indican la ruta de proveedor esperada; la marca real o sintética es la procedencia que acompaña actualmente a cada candidata.',
  },
  warning: { syntheticSymbols: '{count} de {total} símbolos usan datos sintéticos (API no disponible); las candidatas de esos pares están excluidas.' },
  hero: { selected: 'CANDIDATO SELECCIONADO', topSignal: 'CANDIDATO PRINCIPAL', score: 'puntuación {score}/100' },
  stats: { activeSignals: 'Observaciones actuales', buySell: 'Etiquetas direccionales', averageRuleScore: 'Puntuación media', marketBias: 'Sesgo de candidatos' },
  filters: { highRuleScore: 'Puntuación alta (≥80/100)', watchlist: 'Seguimiento' },
  empty: {
    noAssetSignals: 'No hay candidatas de {asset} ahora', noFilteredSignals: 'Ninguna candidata coincide con los filtros actuales',
    ...
    generating: 'Generando candidatas…', analyzing: 'El motor de reglas inspecciona datos de mercado respaldados por proveedores. La primera carga puede tardar hasta 30 segundos.',
    autoRetrying: 'Reintento automático cada 15 s. Las candidatas aparecen cuando las observaciones cumplen el umbral de reglas.',
  ...
  potential: {
    heading: 'Candidatas exploratorias', scoreBand: 'puntuación 50–69/100', count: 'Observaciones: {count}',
    description: 'Observaciones iniciales con acuerdo parcial entre indicadores. Se incluyen para comparar, no para actuar.',
  },
  footer: { tagline: 'Laboratorio TradeClaw · Código abierto · Autoalojado', disclaimer: 'Las candidatas son solo para inspección. No son instrucciones de trading ni asesoramiento financiero.' },
};

const zh: DashboardTranslations = {
  common: {
    all: '全部', buy: '买入', sell: '卖出', live: '已观测', demo: '演示', copied: '已复制！', dismiss: '关闭', refresh: '刷新', pending: '待处理', details: '详情',
    ...
    bias: { bull: '上行假设', bear: '下行假设', neutral: '中性' },
  },
  document: { title: 'TradeClaw — 研究候选列表', titleWithBuyCount: '（{count} 个买入标签）TradeClaw — 候选列表' },
  onboarding: {
  ...
  explanation: {
    title: '为什么出现此候选？', rsiOversold: 'RSI 为 {value}（超卖状态）', rsiBelowNeutral: 'RSI 为 {value}（低于中性）',
  ...
  levels: {
    entry: '观测价格', sl: 'SL', tp1: 'TP1', tp2: 'TP2', tp3: 'TP3', entryHint: '候选生成时记录的供应商价格。',
  ...
  history: {
    heading: '候选结果原始记录', winRate: '胜率', resolved: '已结算', streak: '连续结果', priceMoveSum: '价格变动合计',
    ...
    priceMoveSumHint: '计入候选的 OHLCV 已结算价格变动之算术和，不包含仓位规模或执行成本，不代表投资组合收益。',
    pair: '交易对', direction: '方向', ruleScore: '规则评分', entry: '入场', directionalMove: '方向性变动', viewRecord: '检查完整跟踪记录 →',
  },
  controls: { auto: '60 秒刷新', paused: '刷新已暂停' },
  transparency: {
    providerBefore: '运行时行情优先使用', providerAfter: '并经由市场数据中心。候选 K 线在已配置时使用数据中心，随后加密货币使用 Binance，外汇/贵金属使用 Stooq；合成回退行不会出现在此候选列表中。',
    provenance: '卡片徽章标明预期的数据提供路径；每个候选当前携带的真实/合成质量标记代表运行时数据来源。',
  },
  warning: { syntheticSymbols: '{total} 个品种中有 {count} 个正在使用合成数据（API 不可用）；这些交易对的候选已被隐藏。' },
  hero: { selected: '已选候选', topSignal: '首选候选', score: '规则评分 {score}/100' },
  stats: { activeSignals: '当前观测', buySell: '方向标签', averageRuleScore: '平均规则评分', marketBias: '候选倾向' },
  filters: { highRuleScore: '高规则评分（≥80/100）', watchlist: '关注列表' },
  empty: {
    noAssetSignals: '目前没有{asset}候选', noFilteredSignals: '没有候选符合当前筛选条件',
    ...
    adjustFilters: '当前候选：{count}。调整评分筛选条件即可查看。', showAllAssets: '显示全部资产', generating: '正在生成候选…',
    analyzing: '规则引擎正在检查由提供商支持的市场数据。首次加载最多需要 30 秒。', autoRetrying: '每 15 秒自动重试。观测达到规则阈值时会显示候选。',
  ...
  potential: { heading: '探索性候选', scoreBand: '规则评分 50–69/100', count: '观测数量：{count}', description: '这些早期观测仅有部分指标一致，用于比较而非行动。' },
  footer: { tagline: 'TradeClaw 研究实验室 · 开源 · 自托管', disclaimer: '研究候选仅供检查，不是交易指令，也不构成财务建议。' },
};

const ms: DashboardTranslations = {
  common: {
    all: 'Semua', buy: 'BELI', sell: 'JUAL', live: 'DIPERHATI', demo: 'DEMO', copied: 'Disalin!', dismiss: 'Tutup', refresh: 'Muat semula', pending: 'menunggu', details: 'butiran',
    ...
    bias: { bull: 'Hipotesis naik', bear: 'Hipotesis turun', neutral: 'Neutral' },
  },
  document: { title: 'TradeClaw — Suapan Calon Penyelidikan', titleWithBuyCount: '({count} label BELI) TradeClaw — Suapan Calon' },
  onboarding: {
  ...
  explanation: {
    title: 'Mengapa calon ini muncul?', rsiOversold: 'RSI pada {value} (keadaan terlebih jual)', rsiBelowNeutral: 'RSI pada {value} (di bawah neutral)',
  ...
  levels: {
    entry: 'Harga diperhati', sl: 'SL', tp1: 'TP1', tp2: 'TP2', tp3: 'TP3', entryHint: 'Harga pembekal yang direkod ketika calon dijana.',
  ...
  history: {
    heading: 'Rekod Mentah Hasil Calon', winRate: 'kadar menang', resolved: 'diselesaikan', streak: 'rentetan', priceMoveSum: 'jumlah gerakan harga',
    ...
    priceMoveSumHint: 'Jumlah aritmetik gerakan harga OHLCV bagi calon dikira, tanpa saiz kedudukan atau kos pelaksanaan. Ia bukan pulangan portfolio.',
    pair: 'Pasangan', direction: 'Arah', ruleScore: 'Skor peraturan', entry: 'Kemasukan', directionalMove: 'Gerakan arah', viewRecord: 'Periksa rekod penuh →',
  },
  controls: { auto: 'Muat semula 60s', paused: 'Muat semula dijeda' },
  transparency: {
    providerBefore: 'Sebut harga masa jalan mengutamakan', providerAfter: 'melalui hab data pasaran. Lilin calon menggunakan hab apabila dikonfigurasi, kemudian Binance untuk kripto atau Stooq untuk forex/logam; baris sandaran sintetik tidak dipaparkan dalam senarai ini.',
    provenance: 'Lencana kad mengenal pasti laluan penyedia yang dijangka; tanda kualiti sebenar atau sintetik ialah asal data masa jalan yang kini dibawa oleh setiap calon.',
  },
  warning: { syntheticSymbols: '{count} daripada {total} simbol menggunakan data sintetik (API tidak tersedia); calon bagi pasangan itu disekat.' },
  hero: { selected: 'CALON DIPILIH', topSignal: 'CALON UTAMA', score: 'skor peraturan {score}/100' },
  stats: { activeSignals: 'Pemerhatian semasa', buySell: 'Label arah', averageRuleScore: 'Purata skor peraturan', marketBias: 'Kecenderungan calon' },
  filters: { highRuleScore: 'Skor Peraturan Tinggi (≥80/100)', watchlist: 'Senarai pantau' },
  empty: {
    noAssetSignals: 'Tiada calon {asset} sekarang', noFilteredSignals: 'Tiada calon sepadan dengan penapis semasa',
    ...
    generating: 'Menjana calon…', analyzing: 'Enjin peraturan sedang memeriksa data pasaran bersumber penyedia. Muatan pertama boleh mengambil masa sehingga 30 saat.',
    autoRetrying: 'Cuba semula secara automatik setiap 15 saat. Calon muncul apabila pemerhatian memenuhi ambang peraturan.',
  ...
  potential: { heading: 'Calon eksploratori', scoreBand: 'skor peraturan 50–69/100', count: 'Pemerhatian: {count}', description: 'Pemerhatian awal dengan persetujuan penunjuk separa. Ia untuk perbandingan, bukan tindakan.' },
  footer: { tagline: 'Makmal Penyelidikan TradeClaw · Sumber Terbuka · Dihos Sendiri', disclaimer: 'Calon penyelidikan hanya untuk pemeriksaan. Bukan arahan perdagangan atau nasihat kewangan.' },
};

const ar: DashboardTranslations = {
  common: {
    all: 'الكل', buy: 'شراء', sell: 'بيع', live: 'مرصود', demo: 'تجريبي', copied: 'تم النسخ!', dismiss: 'إغلاق', refresh: 'تحديث', pending: 'قيد الانتظار', details: 'التفاصيل',
    ...
    bias: { bull: 'فرضية صعود', bear: 'فرضية هبوط', neutral: 'محايد' },
  },
  document: { title: 'TradeClaw — قائمة مرشحي البحث', titleWithBuyCount: '({count} تسميات شراء) TradeClaw — قائمة المرشحين' },
  onboarding: {
  ...
  explanation: {
    title: 'لماذا ظهر هذا المرشح؟', rsiOversold: 'قيمة RSI هي {value} (حالة تشبع بيعي)', rsiBelowNeutral: 'قيمة RSI هي {value} (دون المستوى المحايد)',
  ...
  levels: {
    entry: 'السعر المرصود', sl: 'SL', tp1: 'TP1', tp2: 'TP2', tp3: 'TP3', entryHint: 'سعر المزود المسجل عند إنشاء المرشح.',
  ...
  history: {
    heading: 'السجل الخام لنتائج المرشحين', winRate: 'معدل الفوز', resolved: 'محسومة', streak: 'السلسلة', priceMoveSum: 'مجموع حركة السعر',
    ...
    priceMoveSumHint: 'المجموع الحسابي لحركات السعر المحسومة عبر OHLCV للمرشحين المحتسبين، دون أحجام مراكز أو تكاليف تنفيذ. ليس عائداً لمحفظة.',
    pair: 'الزوج', direction: 'الاتجاه', ruleScore: 'درجة القواعد', entry: 'الدخول', directionalMove: 'الحركة الاتجاهية', viewRecord: 'فحص السجل الكامل ←',
  },
  controls: { auto: 'تحديث كل 60 ث', paused: 'التحديث متوقف' },
  transparency: {
    providerBefore: 'تفضّل أسعار وقت التشغيل', providerAfter: 'عبر مركز بيانات السوق. تستخدم شموع المرشحين المركز عند ضبطه، ثم Binance للعملات الرقمية أو Stooq للفوركس والمعادن؛ ولا تظهر صفوف البيانات الاصطناعية الاحتياطية في هذه القائمة.',
    provenance: 'تحدد شارات البطاقات مسار المزوّد المتوقع؛ وتصنيف الجودة الحقيقي أو الاصطناعي هو مصدر بيانات وقت التشغيل المحمول حالياً مع كل مرشح.',
  },
  warning: { syntheticSymbols: 'يستخدم {count} من أصل {total} رمزاً بيانات اصطناعية (واجهة API غير متاحة)؛ لذلك حُجب مرشحو تلك الأزواج.' },
  hero: { selected: 'المرشح المحدد', topSignal: 'المرشح الأبرز', score: 'درجة القواعد {score}/100' },
  stats: { activeSignals: 'الملاحظات الحالية', buySell: 'تسميات الاتجاه', averageRuleScore: 'متوسط درجة القواعد', marketBias: 'اتجاه المرشحين' },
  filters: { highRuleScore: 'درجة قواعد مرتفعة (≥80/100)', watchlist: 'قائمة المتابعة' },
  empty: {
    noAssetSignals: 'لا يوجد مرشحون لـ {asset} الآن', noFilteredSignals: 'لا يوجد مرشحون يطابقون عوامل التصفية الحالية',
    ...
    adjustFilters: 'المرشحون الحاليون: {count}. عدّل تصفية الدرجة لفحصهم.', showAllAssets: 'عرض كل الأصول', generating: 'جارٍ إنشاء المرشحين…',
    analyzing: 'يفحص محرك القواعد بيانات السوق المدعومة من المزوّد. قد يستغرق التحميل الأول حتى 30 ثانية.', autoRetrying: 'تتم إعادة المحاولة تلقائياً كل 15 ثانية. يظهر المرشحون عندما تحقق الملاحظات حد القواعد.',
  ...
  potential: { heading: 'مرشحون استكشافيون', scoreBand: 'درجة القواعد 50–69/100', count: 'الملاحظات: {count}', description: 'ملاحظات أولية بتوافق جزئي بين المؤشرات، للمقارنة لا للتنفيذ.' },
  footer: { tagline: 'مختبر أبحاث TradeClaw · مفتوح المصدر · مستضاف ذاتياً', disclaimer: 'مرشحو البحث للفحص فقط، وليسوا تعليمات تداول أو نصيحة مالية.' },
};
