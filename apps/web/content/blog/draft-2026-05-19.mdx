...
TODO: Present the comparison side by side. Fixed 30-pip stops: win rate ~58%, average win/loss ratio ~1.1, expectancy ≈ +0.08R per trade — barely positive. 2× ATR stops: win rate drops to ~51% (more breathing room means more trades that reverse after the stop), average win/loss ratio climbs to ~1.8, expectancy ≈ +0.42R per trade — 5× better. Net equity curve comparison: ATR-scaled version reaches similar cumulative profit with meaningfully lower maximum drawdown. The tradeoff is explicit: you'll lose more individual trades, but make more money overall. Win rate is a vanity metric; expectancy is the one that matters.
